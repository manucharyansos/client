              ...
              <div>
                <div class="flex my-6">
                  <div v-for="star in stars" :key="star" @click="selectRating(star)">
                    ...
                    <input type="number" :disabled="isDisabled" v-model="rating" hidden>
                  </div>
                  <div v-if="product_data.average_rating">
                    <span class="text-md text-gray-400 ml-12">{{ product_data.average_rating }}</span>
                  </div>
                </div>
                <div>
                  <p v-if="commentError.comment" class="font-sans text-red-500 my-3" v-for="error of commentError.comment">
                    <span >{{error}}</span>
                  </p>
...
<!--            {{ averageRating }}-->
          ...
          <div class="flex flex-wrap -mx-2 mb-12">
            ...
            <div class="w-full md:w-1/3 px-2">
              <a class="flex w-full py-4 px-2 items-center justify-center leading-8 font-heading font-medium tracking-tighter text-xl text-center bg-white dark:text-gray-700 focus:ring-2 focus:ring-gray-200 focus:ring-opacity-50 hover:bg-opacity-60 rounded-xl"
                 href="#">
...
export default {
  ...
  methods: {
    ...mapActions('user', ['reviewProduct']),
    async submitReview() {
      if (!this.$auth.loggedIn){
        this.$router.push('/login')
      }else {
        const response = await this.reviewProduct( {
        ...
        if (response) {
          ...
          this.commentError = this.getErrorMessage.errors
          this.$notify({
            text: this.getErrorMessage.message,
            duration: 3000,
            speed: 1000,
            position: 'center',
            type: 'error',
