<template>
  <div>
    <div id="drawer-update-category" class="fixed top-0 left-0 z-50 w-full h-screen max-w-3xl p-4 overflow-y-auto transition-transform -translate-x-full bg-white dark:bg-gray-800" tabindex="-1" aria-labelledby="drawer-update-category-label" aria-hidden="true">
      ...
      <div class="grid grid-cols-2 gap-4 mt-6 sm:w-1/2">
        <button @click="$emit('updateCategory')" type="submit" class="text-white bg-primary-700 hover:bg-primary-800 focus:ring-4 focus:outline-none focus:ring-primary-300 font-medium rounded-lg text-sm px-5 py-2.5 text-center dark:bg-primary-600 dark:hover:bg-primary-700 dark:focus:ring-primary-800">
          Update category
        </button>
